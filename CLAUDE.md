...
1. Al cargar: `GET /api/v1/deals/:dealId/send-context?userTeam=<equipo>` (templates, contactos, modo jurídico, direccionFiscal, pais) + `GET /api/v1/deals/:dealId/envelope-status`. `userTeam` es el nombre del **primer equipo** del usuario de la card (de `context.user.teams[0]` — el contexto de UI extensions NO expone cuál equipo es el predeterminado, y por regla del cliente cada usuario pertenece a un solo equipo, su país; ese usuario es siempre el propietario del Negocio) — el backend lo usa para filtrar los templates que devuelve. Si el usuario no tiene equipos, el param se omite.
...
3. **No hay campos de Dirección ni País en la card**: el backend los incluye en `send-context` como `direccionFiscal` (propiedad `direccion_fiscal` de la **Empresa** asociada al Negocio) y `pais` (propiedad `pais` del **Negocio**). El backend además envía `fullLocation` (nombre completo del país, ej. "Costa Rica", porque `pais` puede venir como valor interno tipo "CR"). Al API viajan así: `location` = `fullLocation` (fallback: `pais`), `direccionFiscal` = la dirección fiscal de la Empresa, `country` = el `pais` crudo. Si falta alguno: al hacer click en "Enviar" se muestra un error rojo (no se abre el modal) pidiendo llenar la propiedad en HubSpot, con "Volver a comprobar". No hay aviso al cargar.
4. Usuario selecciona Template + Acuerdo + Número de identificación → click "Enviar".
5. `POST /api/v1/docusign/envelopes` con `{ dealId, templateId, contactId, location, direccionFiscal, country, commercialAgreement, legalRepresentative, dniLegalRepresentative }`.
