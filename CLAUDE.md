...
3. **No hay campos de Dirección ni País en la card**: el backend los incluye en `send-context` como `direccionFiscal` (propiedad `direccion_fiscal` de la **Empresa** asociada al Negocio) y `pais` (propiedad `pais` del **Negocio**). Viajan al API como `location` y `country` (texto). Si falta alguno: al hacer click en "Enviar" se muestra un error rojo (no se abre el modal) pidiendo llenar la propiedad en HubSpot, con "Volver a comprobar". No hay aviso al cargar.
